...
class Logger {
    ...
    constructor(logFilePath = null) {
        ...
        if (this.logFilePath) {
            if (!path__default.default.isAbsolute(this.logFilePath)) {
                this.logFilePath = path__default.default.join(process.cwd(), this.logFilePath);
            }
            try {
                fs__default.default.mkdirSync(path__default.default.dirname(this.logFilePath), { recursive: true });
                this.stream = fs__default.default.createWriteStream(this.logFilePath, { flags: "a" });
...
function optionsToConfig(config) {
    ...
    if (config.excludeFrom) {
        const excludeFrom = path__default.default.isAbsolute(config.excludeFrom) ? config.excludeFrom : path__default.default.join(process.cwd(), config.excludeFrom);
        // Verify that the excludeFrom file exists
        if (!fs__default.default.existsSync(excludeFrom)) {
...
function readConfigFromJSON(configFilePath = "") {
    const configPath = configFilePath ? configFilePath : path__default.default.join(process.cwd(), '.incback');
    if (fs__default.default.existsSync(configPath)) {
        const rawData = fs__default.default.readFileSync(configPath, 'utf-8');
        let config;
        try {
